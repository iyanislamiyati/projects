/*
	calculating density matrix and other properties in one-dimensional double well potential.
	additional cases: Helmholtz free energy and entropy.
...
int main(int argc, char *argv[])
{
	...
	//Set potentials
	//Input the potentials value in this line
	/*Any modification can be conducted in the codes below if use other values of potentials,
	especially barrier potentials*/
	...
	//Save
	if((outdata = fopen("fungsi-partisi-dpi-4.dat","w")) == NULL){
		fprintf(stderr," Cannot open file fungsi-partisi-dpi-4.dat\n");
		exit(3);
	}
	for(i = 1; i <= NT; i++){
	...
	if((outdata = fopen("entropi-dpi-4.dat","w")) == NULL){
		fprintf(stderr," Cannot open file entropi-dpi-4.dat\n");
		exit(3);
	}
	for(i = 1; i <= NT; i++){
	...
	if((outdata = fopen("energi-bebas-dpi-4.dat","w")) == NULL){
		fprintf(stderr," Cannot open file energi-bebas-dpi-4.dat\n");
		exit(3);
	}
	for(i = 1; i <= NT; i++){
	...
	if((outdata = fopen("energi-dalam-dpi-4.dat","w")) == NULL){
		fprintf(stderr," Cannot open file energi-dalam-dpi-4.dat\n");
		exit(3);
	}
	for(i = 1; i <= NT; i++){
	...
	if((outdata = fopen("density-matrix-dpi-4.dat","w")) == NULL){
		fprintf(stderr," Cannot open file density-matrix-dpi-4.dat\n");
		exit(3);  
	}
	for(i = 1; i <= NX; i++){
		for(j = 1; j <= NX; j++){
	...
	if((outdata = fopen("densitas-dpi-4.dat","w")) == NULL){
		fprintf(stderr," Cannot open file densitas-dpi.dat\n");
		exit(3);
	}
